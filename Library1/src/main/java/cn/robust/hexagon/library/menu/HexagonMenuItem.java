...
public class HexagonMenuItem {
    ...
    static final int PADDING = 25;//dp
...
//    private int circlePointsCount;
    ...
    HexagonMenuItem(Context context, HexagonMenu menu, int position){
        ...
        Typeface typeface = Typeface.createFromAsset(context.getAssets(), "fonts/SukhumvitSet-Text.ttf");
    ...
    void genPath(){
        ...
        if(mMenu.roundedCorner) {
            ...
            for(int i = 0, j = 2; i < 6; i++, j++){
                j = j % 6;
                k = j * 2 + 1;
                rect.set(circleCenter[j][0] - roundRadius, circleCenter[j][1] - roundRadius, circleCenter[j][0] + roundRadius, circleCenter[j][1] + roundRadius);
                mPath.arcTo(rect, i * 60, 60);
                mPath.lineTo(roundVertexes[k].x, roundVertexes[k].y);
                rect.set(circleCenterPressed[j][0] - roundRadius, circleCenterPressed[j][1] - roundRadius, circleCenterPressed[j][0] + roundRadius, circleCenterPressed[j][1] + roundRadius);
...
//            }
    ...
    protected void onLayout(boolean changed, float length, int paddingLeft, int paddingTop){
        ...
        outer = new Rect((int) vertexes[5].x , (int) vertexes[0].y , (int) vertexes[1].x + 1, (int) vertexes[3].y + 1);
